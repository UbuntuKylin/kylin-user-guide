...
## 系统配置
系统配置模块包括个性化、开机启动、默认应用、时间和日期、用户账户、更新管理器、安全中心七个功能。

### 个性化
“个性化”功能包括桌面背景、主题、字体和锁屏背景。

#### 桌面背景
对当前桌面背景进行设置，选择当前桌面背景的放置方式，对背景列表进行管理。
...
点击“添加”按钮，弹出添加窗口，如图 7所示。
 
![图 7 开机启动添加](image/7.png)

添加有两种方式：

- 手动填写 “程序名称” 和 “执行命令”（注意保证命令合法有效）。
...
### 默认应用
提供系统默认应用的配置项，可修改“网络浏览器”、“邮件阅读器”、“图像查看器”、“音频播放器”、“视频播放器”、“文本编辑器”五种类型的默认打开应用。
...
点击“时区”的下拉列表，会显示所有的时区列表；右侧搜索框可搜索符合关键字的时区。
...
- 管理员可以通过输入自己密码临时提升root权限；标准用户无法提升自己的权限。

- 系统至少需要存在一个管理员用户，最后仅存的管理员用户无法修改用户类型至标准用户。

- 勾选“自动登录”后，每次开机进入该用户不需要输入密码，直接进入用户桌面。

#### 用户选项
设置密码过期时间。
 
![图 14 用户选项](image/14.png)
...
2）点击图18中的“显示”按钮，可显示键盘布局的预览效果，如图 20所示。
...
点击“确定”按钮，新建快捷键被添加到快捷键列表的“自定义”分类，且热键值为“Disable”，如图 23所示。
...
### 网络代理
#### 代理服务器配置
代理模式通常分直接连接到互联网（即无代理）、手动配置代理、自动配置代理。
...
- Http代理可以将代理认证配置在这里，从而免除了访问网络时弹出的认证窗口。点击“细节”按钮，弹出认证界面；勾选“用户认证”选项后，才可以填写“用户名”、“密码”。
...
#### 忽略的主机
“忽略的主机”可以配置忽略主机列表，即访问如下主机时不使用代理。
...
用户还能通过在任务栏上单击网络图标，选择“编辑连接”，打开网络配置。

![图 33 网络连接](image/33.png)

可以编辑已有连接，也可以新增连接（网络类型通常情况下选“以太网”即可）。
