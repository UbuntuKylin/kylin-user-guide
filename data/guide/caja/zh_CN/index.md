...
> 相对路径 —— 从当前所在目录开始的路径，比如位于 **/home** 目录下时，test文件的相对路径为 **kylin/test**
...
| 图标  | 说明  |
| :------------ | :------------ |
| ![](image/a.png)  | ① 转到上一个访问过的位置；② 查看后退历史  |
| ![](image/b.png)  | ① 转到下一个访问过的位置；② 查看前进历史  |
| ![](image/c.png)  | 回到上一级目录  |
| ![](image/d.png)  | 文本框式地址栏，不仅可以输入本机的文件或目录路径，还可以输入一个局域网中共享的文件路径，或是一个ftp地址  |
| ![](image/e.png)  | 搜索栏，搜索用户所需要的文件  |
| ![](image/f.png)  | 设置文件查看方式为 “图标视图” / “列表视图”  |
...
* **卸载 / 弹出**：这两项都是卸载移动设备的作用；区别在于卸载后系统中依然存在该设备（未挂载状态），弹出则无法再在系统中找到该设备。设备右侧的 ![](g.png) 按钮，是弹出的作用。
...
（2）点击 “**创建共享**” 后，文件夹图标会变为![](h.png) ，表示已属于共享文件。
